...
const EditJobDescription: React.FC<UpdateJobDescriptionProps> = ({
  onUpdate,
  data,
  id,
}) => {
  ...
  useEffect(() => {
    if (data && isFirstDialogOpen) {
      ...
      if (employmentValues.includes(data.employmentType as any)) {
        setValue(
          "employmentType",
          data.employmentType as (typeof employmentValues)[number]
        );
      } else {
    ...
    >
      <FormProvider {...methods}>
        ...
        >
          ...
          {/* <InputField label="departmentId" name="departmentId" placeholder="departmentId" />
          ...
          >
            {methods.formState.isSubmitting || isClicked
              ? "Updating..."
              : "Update Job"}
          </Button>
        </form>
