...
import { UpdateApplicationProps } from "@/types/interface";
...
const EditApplication: React.FC<UpdateApplicationProps> = ({ onUpdate, data, id }) => {
  ...
  useEffect(() => {
    if (data && isModalOpen) {
      ...
      // setSelectedFile(data.attachment || null);
    }
  ...
  try {
    ...
    const response = await apiClient.put(`/admin/application/${id}`, payload, {
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 sm:grid-cols-2">
          ...
          <FormItem className="gap-2 grid sm:col-span-2">
            <FormLabel className="text-text text-sm">Attachment</FormLabel>
            <FormControl>
              <div
