...
import AddPermission from "@/components/Modals/AddModals/AddPermission";
...
const SettingPermission = () => {
  ...
  // --- State Declarations ---
  const [permission, setPermission] = useState<UserProps[]>([]);
  ...
  // --- Memoized Values ---
  const allCards = useMemo(() => permission.map((data) => data.id), [permission]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (page === 1) {
          setPermission(newUsers);
        } else {
          setPermission((prev) => [...prev, ...newUsers]);
        }
  ...
  const handleDelete = (id: string) =>
    setPermission((prev) => prev.filter((data) => data.id !== id));
  const handleAddDescription = (newData: UserProps) => {
    if (newData) setPermission((prev) => [newData, ...prev]);
  };
   ...
   <div
      ...
      >
        <Operations
          ...
          serverSearchPlaceholder="Search all Permission..."
        />

        <div className="w-full sm:w-auto">
          <AddPermission onAdd={handleAddDescription} />
        </div>
      </div>
        ...
        <div className="divide-y divide-gray-100">
          {loading && page === 1 ? (
            ...
            permission.map((user, index) => {
              const isLastElement = permission.length === index + 1;
              const ref =
          ...
          {!hasMore && !loading && permission.length > 0 && (
