...
const statusOptions = ["APPLIED", "SHORTLIST", "INTERVIEW", "OFFERED", "HIRED", "REJECTED"] as const;
...
const ResumePreviewModal = ({ 
  ...
  // Check if it's a PDF using file type OR URL extension
  const isPDF = fileType === 'application/pdf' || previewUrl.toLowerCase().endsWith(".pdf");
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="relative bg-white rounded-lg shadow-lg max-w-5xl w-full max-h-[90vh] overflow-hidden">
            ...
            {isPDF ? (
              <iframe src={previewUrl} className="w-full h-[90vh]" title="Full Resume" />
            ) : (
              <img src={previewUrl} alt="Full Resume" className="w-full h-auto object-contain" />
            )}
          </div>
        </div>
      )}
...
const UpdateApplicationRoute = () => {
  ...
  useEffect(() => {
    ...
    const fetchApplication = async () => {
      try {
        setIsLoading(true);
        const response = await apiClient.get(`/application/${applicationId}`);
        const fetchedApp = response.data?.application || response.data?.data || response.data;

        if (fetchedApp) {
          reset({
            jobId: fetchedApp.jobId || "",
            companyId: fetchedApp.companyId || companyId,
            candidateName: fetchedApp.candidateName || "",
            email: fetchedApp.email || "",
            phone: fetchedApp.phone || "",
            currentCTC: fetchedApp.currentCTC || "",
            expectedCTC: fetchedApp.expectedCTC || "",
            noticePeriod: fetchedApp.noticePeriod || "",
            status: fetchedApp.status || "APPLIED",
            source: fetchedApp.source || "",
            skills: fetchedApp.skills || [],
            experience: fetchedApp.experience || [],
          });

          if (fetchedApp.resumeUrl) {
            const fullUrl = fetchedApp.resumeUrl.startsWith("http")
              ? fetchedApp.resumeUrl
              : `${BASE_URL}${fetchedApp.resumeUrl}`;
            setPreviewUrl(fullUrl);
            setFileType(null); // Existing file from server
          }
        }
      } catch (error: any) {
        toast.error("Failed to fetch application data.");
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key === 'skills' && Array.isArray(value)) {
          ...
          payload.append(key, JSON.stringify(value));
        } else if (value !== undefined && value !== null && key !== 'resume') {
  ...
  return (
    <>
      <DynamicBreadcrumb
        links={[
          { label: "Applications", href: `/companies/${companyId}?tab=application` },
          { label: "Edit Application" },
        ]}
      />
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-[calc(100vh-105px)] overflow-y-auto gap-5">
          <section className="bg-white border border-gray-200 rounded-xl p-4 grid gap-5">
            <h2 className="text-xl font-bold">Edit Application Details</h2>
            ...
            {isLoading ? (
              ...
              <>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Combobox
                            placeholder="Select Job"
                            options={description.map(d => ({ value: d.id, label: d.name }))}
                            value={field.value}
                            onSelect={field.onChange}
                          />
                        </FormControl>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <InputField label="Current CTC" name="currentCTC" placeholder="Enter CTC" />
                  <InputField label="Expected CTC" name="expectedCTC" placeholder="Enter CTC" />
                </div>

                <div className="grid sm:grid-cols-2 gap-5">
                  <InputField label="Notice Period" name="noticePeriod" placeholder="Enter period" />
                  <SelectField label="Status" name="status" placeholder="Select status" options={statusSelect} />
                </div>
                ...
                <div>
                  ...
                  {fields.map((item, index) => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 mb-2 p-3 border rounded">
                      <div className="col-span-4">
                        <InputField label="Company" name={`experience.${index}.company`} placeholder="Company" />
                      </div>
                      <div className="col-span-4">
                        <InputField label="Role" name={`experience.${index}.role`} placeholder="Role" />
                      </div>
                      <div className="col-span-3">
                        <InputField label="Years" name={`experience.${index}.years`} placeholder="Years" />
                      </div>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="flex gap-4">
                      <div
                        className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer flex-1"
                      ...
                      {previewUrl && <ResumePreviewModal previewUrl={previewUrl} fileType={fileType} />}
                    </div>
                  </FormControl>
                </FormItem>
